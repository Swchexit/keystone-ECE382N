...
class Verifier {
 ...
 private:
  ...
  // Computes the expected hash_history by iterating through the connection log.
  // For each connected enclave, it computes the initial hash and extends.
  void compute_expected_hash_history(byte* expected_hash, Report& report);
